...
with st.sidebar:
    ...
    news_enabled = st.toggle("📰 Récupération des news", value=True)
    analysis_enabled = st.toggle("🤖 Analyse Mistral IA", value=True)
    if st.button("🔄 Forcer récup news"):
        st.session_state.last_news_time = 0.0
        st.rerun()
    if st.button("🔄 Forcer analyse IA"):
...
if news_enabled and time.time() - st.session_state.last_news_time >= NEWS_INTERVAL:
...
if analysis_enabled and time_since >= ANALYSIS_INTERVAL:
